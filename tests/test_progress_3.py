...
@pytest.mark.parametrize("favorability, expected_choice, expected_favor_change", [
    (70, "冷静分析证据", 2),   # 高好感度 → +2
    (50, "冷静分析证据", 1),   # 中等好感度 → +1
    (40, "冷静分析证据", 0),   # 低好感度 → 0
])
def test_progress_3_analyze_evidence(mocker, favorability, expected_choice, expected_favor_change):
    """
    测试玩家选择【冷静分析证据】时的输出。
    """
    mocker.patch("builtins.input", return_value="1")  # 模拟用户输入 "1"

    result = progress_3_event(favorability)
...
def test_progress_3_question_godot(mocker, favorability, expected_choice, expected_favor_change):
    """
...
@pytest.mark.parametrize("favorability", [70, 50, 40])
def test_progress_3_defend_client(mocker, favorability):
    """
    测试玩家选择【坚定维护委托人】时的输出。
    """
